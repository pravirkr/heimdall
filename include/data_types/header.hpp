...
public:
    ...
    SigprocHeader()
        : az_start(0.0), za_start(0.0), src_raj(0.0), src_dej(0.0), tstart(0.0),
          ...
          nifs(0), npuls(0), refdm(0.0), signed_data(0), size(0) {}
};
...
void header_write(
    BinaryStream& stream, double raj, double dej, double az, double za) {
...
bool read_header(BinaryStream& stream, Header& header) {
    ...
    while (true) {
        ...
        if (s == "HEADER_END")
            ...
            std::cerr << "Warning: read_header: unknown parameter " << s
                      << std::endl;
        }
    }
    ...
    if (0 == header.nsamples) {
        ...
        header.nsamples =
            (total_size - header.size) / header.nchans * 8 / header.nbits;
