...
void add_white_noise(float *data,
                     size_t count,
                     float  mean = 0.0,
                     float  rms  = 1.0) {
...
void add_brown_noise(float *data,
                     size_t count,
                     float  mean = 0.0,
                     float  rms  = 1.0) {
    float z = 0.0;
...
void add_red_noise(
    float *data, size_t count, size_t min_k, size_t max_k, float amp) {
...
void add_giant(float *data,
    ...
    */
    ...
    for (size_t i = first_bin - 1; i <= last_bin + 1; ++i) {
        ...
        float overlap_time =
            std::max(overlap_end_time - overlap_start_time, 0.f);
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 1) {
        ...
        cout << "About: Generates filterbank data containing pre-specified "
                "giants."
             << endl;
        ...
        cout << "-n -nbits int\tNumber of bits per sample for output data"
             << endl;
        ...
        cout << endl;
        cout << "Input files should list one giant per line as follows:"
             << endl;
    ...
    if (red_noise) {
        ...
        for (size_t c = 0; c < nchans; ++c) {
            ...
            std::transform(time_series, time_series + nsamps,
                           &red_noise_vals[0], time_series, std::plus<float>());
        }
    }
    ...
    */
    ...
    for (size_t c = 0; c < nchans; ++c) {
        ...
        for (size_t g = 0; g < giants.size(); ++g) {
            ...
            add_giant(time_series, nsamps, nchans, c, giants[g].time,
                      giants[g].width,
                      // Note: We normalise the flux such that the given
                      //         values matches what will be detected under
                      //         ideal circumstances.
                      giants[g].flux / sqrt(nchans) * sqrt(nbins), giants[g].DM,
                      dt, f0, df);
        }
    }
