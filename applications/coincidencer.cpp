        ...
        if (nfiles > 0) {
            ...
            while (waiting_for_beams) {
                ...
                try {
                    ...
                    iss >> utc_start >> first_sample_utc >>
                        first_sample_index >> beam_number >> num_events >> ws;
                    ...
                    // check first_sample_utc to see if it matches an existing
                    // chunk
                    if (curr_first_sample_utc.empty() ||
                        curr_first_sample_utc.compare(first_sample_utc) != 0) {
                        curr_chunk      = -1;
                        time_t youngest = 0;
                        for (unsigned ichunk = 0; ichunk < chunks.size();
                             ichunk++) {
                            // get the relative age between this new beam/chunk
                            // and the existing ones
                            time_t relative_age =
                                chunks[ichunk]->get_relative_age(
                                    first_sample_utc);
                        ...
                        // if the new beam's data preceeds the youngest chunk,
                        // discard it
                        if (youngest < 0)
                            curr_chunk = -2;
                        ...
                        if (curr_chunk >= 0) {
                            if (verbose > 1)
                                cerr << "main: found existing chunk for "
                                     << first_sample_utc << endl;
                        }
                        // if not, create a new chunk
                        if (curr_chunk == -1) {
                            if (verbose > 1)
                                cerr << "main: creating new chunk for "
                                     << first_sample_utc << endl;
                    ...
                    if (curr_chunk >= 0) {
                        if (verbose > 1)
                            cerr << "main: chunks[" << curr_chunk
                                 << "]->addBeam(" << beam_number << ")" << endl;
                        chunks[curr_chunk]->addBeam(
                            utc_start, first_sample_utc, first_sample_index,
                            beam_number, num_events, iss);

                        // if we have reached the specified number of beams for
                        // this chunk, or if more than 3 chunks are stored, dump
                        // a chunk
