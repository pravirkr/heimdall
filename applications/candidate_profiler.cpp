...
void exclusive_scan(InputIterator  begin,
                    InputIterator  end,
...
int normalise_series_array(float *out, size_t nsamps, size_t nseries) {
    ...
    for (size_t d = 0; d < nseries; ++d) {
        ...
        */
        ...
        // Normalise
        std::transform(&out[i], &out[i + nsamps], buf.begin(),
                       abs_deviation(0.));
        std::nth_element(buf.begin(), buf.begin() + buf.size() / 2, buf.end());
...
int gen_freq_time_plot(string filename,
    ...
    if (verbose) {
        ...
        cerr << "Input sample range: " << first_samp << " : "
             << first_samp + in_nsamps << " = " << in_nsamps << " samples"
             << endl;
        cerr << "DM = " << dm << endl;
    }
...
int gen_dm_time_plot(string       filename,
    ...
    */
    ...
    size_t nchan_bytes =
        header.nchans * header.nbits / (8 * sizeof(dedisp_byte));
    ...
    if (error != DEDISP_NO_ERROR) {
        cerr << "dedisp_execute failed: " << dedisp_get_error_string(error)
             << endl;
        return -1;
    }
...
void usage(char *binary) {
    ...
    cerr << "    dmlist              file containing list of DMs to plot"
         << endl;
    cerr << "    samp                sample index of the candidate" << endl;
    cerr << "    filter              filter (log2) in which candidate was found"
         << endl;
    cerr << "    dm_idx              line in dmlist which corresponds to "
            "nominal DM for candidate"
         << endl;
...
int main(int argc, char *argv[]) {
    ...
    error = gen_dm_time_plot(input_name, &dm_list[0], dm_list.size(), samp,
                             filter, 0, out_nsamps, 1, &dm0_time_data[0],
                             do_filter, verbose);
    if (error) {
    ...
    size_t max_idx =
        std::max_element(dm_time_data.begin(), dm_time_data.end()) -
        dm_time_data.begin();
    if (verbose) {
        ...
        cerr << " Dist from given = "
             << abs(max_idx % out_nsamps - out_nsamps / 2) << endl;
    }
