...
std::ostream &operator<<(std::ostream &os, const Candidate *c) {
    os << c->snr << "\t" << c->sample_idx << "\t" << c->sample_time << "\t"
       ...
       << c->members << "\t" << c->begin << "\t" << c->end << "\t" << c->nbeams
...
CandidateChunk::CandidateChunk(int argc, int optind, char **argv) {
    ...
    for (unsigned int i = 0; i < n_beams; i++) {
        if (verbose)
            cerr << "CandidateChunk::CandidateChunk opening file "
                 << argv[optind + i] << endl;
        ...
        if (verbose)
            cerr << "CandidateChunk::CandidateChunk parsed beam number as "
                 << beam << endl;
...
// add beam
void CandidateChunk::addBeam(string              _utc_start,
                             string              _first_sample_utc,
